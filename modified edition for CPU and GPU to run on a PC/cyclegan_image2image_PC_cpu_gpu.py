...
"""### Подключаем ниже все необходимые библиотеки"""
...
"""### Для работы подключаем свою папку по-умолчанию"""

##### Для работы подключаем свою папку по-умолчанию #####

pr_0 = str("Узнаем нашу директорию по-умолчанию и присваиваем её значение переменной DIR")
print(pr_0)

DIR = os.getcwd() # узнаем нашу директорию по-умолчанию и присваиваем её значение переменной DIR
print(DIR)
print()
...
# которые далее автоматически распаковываются в папку horse2zebra_ с нашими изображениями в трейновых и тестовых подпапках (делается ниже автоматически).

pr_1 = str("--> В директории по-умолчанию должны находиться вместе python-файл (либо ноутбук), веса обученных моделей и три части архива horse2zebra_*.zip,")
pr_2 = str("которые далее автоматически распаковываются в папку horse2zebra_ с нашими изображениями в трейновых и тестовых подпапках (делается ниже автоматически).")
print(pr_1)
print(pr_2)
print()
...
pr_4 = str("Распаковываем наш архив в подпапку horse2zebra_")
print(pr_4)

# распаковываем наш архив в подпапку horse2zebra_
for fn in zips:
    print(fn)
...
####### class dataset для обработки изображений из нашего датасета horse2zebra_ #######
...
### Применяем, если были сбросы при обучении моделей или хотим вернуться к ранее сохраненному состоянию нашей модели !!!
...
if device == torch.device('cuda'):
    GA2B.load_state_dict(torch.load(GA2B_DIR)) # gpu
    GB2A.load_state_dict(torch.load(GB2A_DIR)) # gpu
    D1.load_state_dict(torch.load(D1_DIR)) # gpu
    D2.load_state_dict(torch.load(D2_DIR)) # gpu
    
elif device == torch.device('cpu'):
    GA2B.load_state_dict(torch.load(GA2B_DIR, map_location=torch.device('cpu'))) # cpu
    GB2A.load_state_dict(torch.load(GB2A_DIR, map_location=torch.device('cpu'))) # cpu
    D1.load_state_dict(torch.load(D1_DIR, map_location=torch.device('cpu'))) # cpu
    D2.load_state_dict(torch.load(D2_DIR, map_location=torch.device('cpu'))) # cpu
...
def train(GA2B, GB2A, D1, D2, num_epochs, data_train_A, data_train_B, batch_size, lamb, loss_g, loss_d):
    ...
    for epoch in range(num_epochs):
        ...
        n_ep = str(epoch + 1)
        print(str("Идет обучение CycleGAN на " + n_ep + "эпохе." + " Это может занять долгое время..."))
        print(str("Особенно, если обучение CycleGAN производится на CPU."))
        print()
        ...
        for x, Y in zip(data_train_A, data_train_B):  
            ...
            f = np.rollaxis(f, axis=2)
            # axes[0].imshow(f)
            if (epoch == num_epochs - 1) and (j == len(data_train_A) - 1):
                axes[0].imshow(f)
            # if (j == len(data_train_A) - 1):
            #    axes[0].imshow(f)
            ...
            i = (i + 1) % batch_size
            loss[0].append(gen_losses) # loss генератора для каждого батча
            # print(torch.mean(torch.FloatTensor(loss[0]))) # вывод loss генератора для каждого батча
            loss[1].append((l_d1 + l_d2) / 2) # loss дискриминаторов для каждого батча
            
            j = j + 1


        losses[0].append(torch.mean(torch.FloatTensor(loss[0]))) # средний loss генератора для каждой эпохи
        losses[1].append(torch.mean(torch.FloatTensor(loss[1]))) # средний loss дискриминаторов для каждой эпохи

        ## print(epoch)
        
        tic = time()
        print('* Epoch %d/%d' % (epoch+1, num_epochs)) # вывод текущей эпохи и общего количества эпох

        # plt.plot(losses[0])
        print(losses[0]) # вывод среднего loss генератора для каждой эпохи
        print()
...
def test(GA2B, GB2A, D1, D2, num__test_epochs, data_test_A, data_test_B, batch_size, lamb, loss_g, loss_d):
    ...
    for epoch in range(num_test_epochs):
        ...
        n_ep = str(epoch + 1)
        print(str("Идет валидация CycleGAN на " + n_ep + " эпохе." + " Это может занять долгое время..."))
        print(str("Особенно, если валидация CycleGAN производится на CPU."))
        print()
        ...
        for x, Y in zip(data_test_A, data_test_B):
            ...
            # with torch.no_grad():
                ...
                f = np.rollaxis(f, axis=2)
                # axes[0].imshow(f)
                # if (epoch == num_test_epochs - 1) and (j == len(data_test_A) - 1):
                #    axes[0].imshow(f)
                if (j == len(data_test_A) - 1):
                    axes[0].imshow(f)
                ...
                i = (i + 1) % batch_size
                loss_test[0].append(gen_losses) # loss генератора для каждого батча
                # print(torch.mean(torch.FloatTensor(loss_test[0]))) # вывод loss генератора для каждого батча
                loss_test[1].append((l_d1 + l_d2) / 2) # loss дискриминаторов для каждого батча
                
                j = j + 1


        losses_test[0].append(torch.mean(torch.FloatTensor(loss_test[0]))) # средний loss генератора для каждой эпохи
        losses_test[1].append(torch.mean(torch.FloatTensor(loss_test[1]))) # средний loss дискриминаторов для каждой эпохи

        ## print(epoch)
        
        tic = time()
        print('* Epoch %d/%d' % (epoch+1, num_test_epochs)) # вывод текущей эпохи и общего количества эпох

        # plt.plot(losses_test[0]) 
        print(losses_test[0]) # вывод среднего loss генератора для каждой эпохи
        print()
...
def test_new(GA2B, GB2A, D1, D2, data_test_A, data_test_B, batch_size, cycle_images):
    ...
    # fig, axes = plt.subplots(nrows=k, ncols=6, figsize=(12, k*6))
    
    # fg = plt.figure(figsize=(12, k*6), constrained_layout=True)
    fg = plt.figure(figsize=(12, k*6), constrained_layout=False)
    gs = fg.add_gridspec(k, 6)
    ...
    for x, Y in random.sample(c, k=cycle_images):
        ...
        # with torch.no_grad():
            ...
            # fig, axes = plt.subplots(nrows=j, ncols=6, figsize=(12, 6))
            
            f = x[i].cpu().detach().numpy() / 2 + 0.5
            f = np.rollaxis(f, axis=2)
            f = np.rollaxis(f, axis=2)
            # axes[0].imshow(f)
            fig_ax_0 = fg.add_subplot(gs[j, 0])
            plt.imshow(f)
            ...
            f = np.rollaxis(f, axis=2)
            # axes[1].imshow(f)
            fig_ax_1 = fg.add_subplot(gs[j, 1])
            plt.imshow(f)
            ...
            f = np.rollaxis(f, axis=2)
            # axes[2].imshow(f)
            fig_ax_2 = fg.add_subplot(gs[j, 2])
            plt.imshow(f)
            ...
            f = np.rollaxis(f, axis=2)
            # axes[3].imshow(f)
            fig_ax_3 = fg.add_subplot(gs[j, 3])
            plt.imshow(f)
            ...
            f = np.rollaxis(f, axis=2)
            # axes[4].imshow(f)
            fig_ax_4 = fg.add_subplot(gs[j, 4])
            plt.imshow(f)
            ...
            f = np.rollaxis(f, axis=2)
            # axes[5].imshow(f)
            fig_ax_5 = fg.add_subplot(gs[j, 5])
            plt.imshow(f)
...
"""### Цикл выбора этапа обучения, валидации либо тестирования наших моделей"""
...
str_1 = str("Введите ниже цифру от 1 до 4: 1 - обучение модели, 2 - валидация модели, 3 - тестирование модели, 4 - EXIT ")
str_2 = str("Если у вас нет GPU, а только CPU, то СОВЕТУЮ ввести цифру 3 - ТЕСТИРОВАНИЕ МОДЕЛИ или цифру 4 - EXIT, если не хотите тратить своё время и ждать результатов обучения или валидации модели")

print(str_1)
print(str_2)
print()

str_tv = str("input number (введите цифру от 1 до 4: 1 - TRAIN, 2 - VAL, 3 - TEST, 4 - EXIT): ")
...
while tv < 4:
    ...
    if tv == 1:
        ...
        # data_train_A = DataLoader(dataset_train_A, batch_size = batch_size, shuffle = False)
        # data_train_B = DataLoader(dataset_train_B, batch_size = batch_size, shuffle = False)
        ...
        print('Далее на картинках, которые появятся ниже, расположены изображения в следующем порядке (слева-направо):')
